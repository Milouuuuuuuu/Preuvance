...
- Un digest de dépendances peut être fourni. Traite-le comme une observation technique bornée : il peut étayer l’usage d’un SDK, d’un framework ou d’une base vectorielle, mais il ne prouve ni la finalité métier, ni le rôle juridique, ni les personnes concernées. N’invente rien au-delà des packages listés.
- Un digest agrégé du scan local peut aussi être fourni avec consentement. Il ne contient ni chemin, ni IP, ni processus, ni contenu. Utilise-le pour signaler la concordance ou une divergence d’usage, jamais pour déduire seul une qualification juridique.
...
export function buildExtractionInput(request: AssessmentRequest): string {
  const dependencyObservation = request.dependencyDigest
    ? JSON.stringify(
        {
          schemaVersion: request.dependencyDigest.schemaVersion,
          scannedAt: request.dependencyDigest.scannedAt,
          dependencies: request.dependencyDigest.dependencies,
          coverage: request.dependencyDigest.coverage,
          warnings: request.dependencyDigest.warnings,
        },
        null,
        2,
      )
    : "Aucun digest de dépendances joint.";
